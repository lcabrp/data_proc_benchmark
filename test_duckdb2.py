...
print("Data loaded. Running QUALIFY with ROW_NUMBER query...")
...
SELECT d.*, s.total_bytes,
       ROW_NUMBER() OVER (PARTITION BY d.event_type ORDER BY s.total_bytes DESC) AS total_rank
FROM d
...
t1 = time.time()
print(f'Qualify time with ROW_NUMBER: {t1-t0:.2f}s, rows: {len(r)}')

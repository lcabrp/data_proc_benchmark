...
def read_dataset(ds: Path):
    ...
    try:
        ...
        logging.error(f"Error reading dataset from {ds}: {e}")
        sys.exit(1)
...
def benchmark_scenario_a():
    ...
    logging.info(f"  Load time: {load_time:.3f}s")
    logging.info(f"  DataFrame Deep Memory Size: {deep_mem:.2f} MB")
    logging.info(f"  Process Memory RSS delta: {process_mem_delta:.2f} MB")
...
def main():
    if not DATASET_PATH.exists():
        logging.error(f"Error: Dataset {DATASET_PATH} not found. Please run generator or update path.")
        sys.exit(1)
        
    logging.info(f"Benchmarking dataset: {DATASET_PATH}")
    logging.info(f"Dataset File Size: {DATASET_PATH.stat().st_size / (1024**2):.2f} MB")
    ...
    logging.info("\n" + "="*70)
    logging.info("COMPARATIVE RESULTS: VANILLA VS OPTIMIZED PANDAS")
    logging.info("="*70)
    ...
    for key, label, direction, fmt in metrics:
        ...
        logging.info(f"{label:<25} | {val_v:>{18},{fmt}} | {val_o:>{18},{fmt}} | {impr_label:>12}")
